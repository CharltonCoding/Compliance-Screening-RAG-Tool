...
/**
 * API Route: /api/mcp/market-data
 *
 * Proxies requests to the Python MCP server's get_market_data tool.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${mcpServerUrl}/tools/get_market_data`, {
      method: 'POST',
      headers: {
    ...
    console.error('Error calling MCP get_market_data:', error);
    return NextResponse.json(
      {
        error: true,
